...
Read [`automation/qa-prompt.template.md`](../../automation/qa-prompt.template.md) — the canonical prompt body. Substitute the placeholders below, then pass the result as `$ARGUMENTS` to `/codex` via the `SlashCommand` tool. The template's `Required output format:` block tells Codex to emit the three structured sections plus the `COUNT_BLOCKER=... COUNT_NIT=...` trailer (parsed by `automation/qa-lib.sh:extract_qa_counts`).
...
Codex returns its full audit. Do not paraphrase, do not silently filter. The full block goes into the Pass-3 report verbatim.
...
- **Refusing to invoke `/codex` after the self-report is a bug.** The audit *is* the second pass. If `/codex` is unavailable, refuse the whole run up front — do not write a self-report-only file and pretend it's an audit.
