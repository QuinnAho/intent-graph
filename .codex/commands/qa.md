...
In the Claude-Code-driven workflow that is the primary use case, the parent is Claude Code and the auditor is invoked via `/codex` (which wraps `codex exec --sandbox read-only`). See `.claude/commands/qa.md` for the canonical body.
