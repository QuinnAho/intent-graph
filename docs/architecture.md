...
- **Thirteen slash commands** — `/intentgraph-init`, `/intentgraph-status`, `/intentgraph-lift`, `/intentgraph-spec`, `/intentgraph-adr`, `/intentgraph-verify`, `/intentgraph-ralph`, `/intentgraph-ralph-run`, `/intentgraph-ralph-resume`, `/intentgraph-ralph-status`, `/intentgraph-ralph-cancel`, `/codex`, `/qa`. Authoritative list lives under `.claude/commands/` — count from there if this paragraph drifts.
